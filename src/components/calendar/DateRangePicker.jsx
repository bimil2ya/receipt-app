...
export default function DateRangePicker({ startDate, endDate, onChange }) {
  ...
  return (
    <div className="bg-slate-900 rounded-2xl border border-slate-700 p-3">
      {/* 안내 텍스트 */}
      <p className={`text-xs font-bold mb-3 text-center ${hint.color}`}>{hint.text}</p>

      {/* 월 이동 헤더 */}
      <div className="flex items-center justify-between mb-3 px-1">
        <button
          onClick={prevMonth}
          className="w-8 h-8 flex items-center justify-center rounded-xl bg-slate-800 text-slate-300 font-black active:scale-95"
        >‹</button>
      ...
      <div className="grid grid-cols-7 mb-1">
        {DAY_LABELS.map((d, i) => (
          <div
            key={d}
            className={`text-center text-[11px] font-black py-1 ${i === 0 ? 'text-red-400' : 'text-slate-500'}`}
          >{d}</div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-7">
        ...
        {Array.from({ length: daysInMonth }, (_, i) => i + 1).map(day => {
          ...
          return (
            <div
              key={day}
              className={`relative h-10 flex items-center justify-center ${cellBg}`}
            >
