export default function TripWorkflowTabs({ activePanel, onBudget, onInput, onManagement }) {
  const buttonClass = (active) => `rounded-2xl px-2 py-2 text-[13px] font-black transition-all active:scale-95 min-h-[40px] flex items-center justify-center gap-1.5 whitespace-nowrap ${
    active
  ...
  return (
    <div className="grid grid-cols-3 gap-2">
      <button
        onClick={onBudget}
        className="rounded-2xl px-2 py-2 text-[13px] font-black transition-all active:scale-95 min-h-[40px] bg-slate-800 border border-slate-700 text-slate-300 hover:bg-slate-700 flex items-center justify-center gap-1.5 whitespace-nowrap"
      >
      ...
      <button onClick={onInput} className={buttonClass(activePanel === 'input')}>
        <span className={stepClass(activePanel === 'input')}>2</span>
        <span>입력</span>
      </button>
      <button onClick={onManagement} className={buttonClass(activePanel === 'management')}>
