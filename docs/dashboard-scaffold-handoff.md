...
## 코드 리뷰 상태 (2026-09-11, 커밋 `1d459fb` + `db43834`)

내부 리뷰 1회 + 자체 재검토 1회 완료. 판정: **Codex에게 넘겨도 되는 상태** —
`_dashboardData.js` 실구현은 발견된 어떤 이슈로도 차단되지 않는다.
- `1d459fb`: 내부 리뷰 반영(fail-closed 기본값, x-real-ip, PIN 경고, expire NX 등)
- `db43834`: 자체 재검토 반영(api.js 네트워크 실패 방어, recentMonths 월 롤오버 버그, preview도 fail-closed)
